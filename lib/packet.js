...
var data_types = {
  ...
  'UInt8Array': {
    ...
    serialize: function (input, buff, ind) {
      ...
      for (var i = 0; i < len; i++) {
        buff.writeUInt8(input[i], ind+1+i);
      }
      return len+1;
    }
  },
  'Int32Array': {
    ...
    serialize: function (input, buff, ind) {
      ...
      buff.writeUInt8(len, ind);
      for (var i = 0; i < len; i++) {
        buff.writeInt32BE(input[i], ind+1+i*4);
      }
      return len*4+1;
    }
  },
...
var PacketByteContainer = function(src) {
  ...
  this.bytes = function(start, end) {
    if (start === undefined && end === undefined) return this.all_bytes();
...
var PacketAssembler = function (offset) {
  ...
  this.bytes = function (start, end) {
    return new ByteRange(this, start, end, this.offset);
  };

  this.byte = function (start) {
    return new ByteRange(this, start, start, this.offset);
  };
  ...
  var ByteRange = function (assemlber, start, end, offset) {
    ...
    */
     ...
     * the buffer and the size of the ByteRange should
     * be the same
     */
    ...
    this.fromRawUInt8Array = function (val) {
      var ind = self.start;
      this.dest.instructions.push(function (buff) {
        var len = val.length;
        for (var i = 0; i < len; i++) {
          buff.writeUInt8(val[i], ind+1+i);
        }
        return len;
      });
    };
  ...
  this.assemble = function () {
    ...
    this.instructions.forEach(function (func) {
      func(buff);
    });
    return buff;
  };
  ...
  this.append = function (assmblr) {
    ...
    var new_len = assmblr.offset + assmblr.length;
